    ...
    <div>
      <Title>
        ...
        <span style={{ fontSize: '1.5em' }}>
          {club.favorite_count} <i className={(props.favorites.includes(club.id) ? 'fa' : 'far') + ' fa-heart'} style={{ cursor: 'pointer' }} onClick={() => props.updateFavorites(club.id) ? club.favorite_count++ : Math.max(0, club.favorite_count--)}></i>
