...
class SearchBar extends React.Component {
  ...
  render () {
    const {
      tagOptions, sizeOptions, applicationOptions, selectedTags,
    } = this.state
    const { switchDisplay, resetDisplay, updateTag } = this.props
    return (
      <div style={{
        height: '100vh', width: '100%', overflow: 'hidden', position: 'sticky', top: -20,
      }}
      >
        <div
          style={{
        ...
        >
          <div className="is-flex" style={{ justifyContent: 'space-between', padding: '0 3px' }}>
            ...
            <div className="is-flex">
              ...
              >
                <span className="icon">
                  <i className="fas fa-list" style={{ cursor: 'pointer', color: CLUBS_GREY }} onClick={e => switchDisplay('list')} />
                </span>
              </Pop>
            </div>
          </div>
          <div style={{ margin: '30px 0' }}>
            <hr style={{
              backgroundColor: CLUBS_GREY, height: '2px', margin: 0, padding: 0,
            }}
            />
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 3px' }}>
              ...
              <span className="icon" style={{ cursor: 'pointer', color: CLUBS_GREY }}>
                {this.state.nameInput ? <i onClick={e => this.setState({ nameInput: '' })} className="fas fa-times" /> : <i className="fas fa-search" />}
              </span>
            </div>
          </div>
          <DropdownFilter
