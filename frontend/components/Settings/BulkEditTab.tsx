...
const BulkEditTab = ({ tags, clubfairs, affiliations }: BulkEditTabProps) => {
  const canManageClubs = apiCheckPermission('clubs.manage_club')
  const canManageFairs = apiCheckPermission('clubs.see_fair_status')
  const canSendEmailBlast = apiCheckPermission('clubs.send_club_email_blast')
  ...
  return (
    <>
      <div className="box">
        ...
        {!canSendEmailBlast ? (
          <Text>You do not have permission to send club email blasts.</Text>
        ) : (
          <>
            <Text>
              Sends an email blast to the selected group of users. Select a
              target group and provide the message content. All active users
              with roles equivalent to or higher than the selected group will be
              notified. Leave the subject blank to use the default subject line.
            </Text>
            <Formik
              initialValues={{
              ...
              onSubmit={async (values, { setSubmitting, resetForm }) => {
                try {
                  ...
                  toast.error(
                    'Failed to send email blast. Please check the logs.',
                  )
                } finally {
                  setSubmitting(false)
                }
      ...
      <div className="box">
        ...
        {!canManageClubs ? (
          <Text>You do not have permission to bulk edit clubs.</Text>
        ) : (
          <>
            <Text>
              You can use the form below to perform bulk editing on{' '}
              {OBJECT_NAME_PLURAL}. Specify the list of {OBJECT_NAME_SINGULAR}{' '}
              codes below, which tags or affiliations you want to add or remove,
              and then press the action that you desire.
            </Text>
      ...
      <div className="box">
        ...
        {!canManageFairs ? (
          <Text>You do not have permission to manage fair events.</Text>
        ) : (
          <>
            <Text>
