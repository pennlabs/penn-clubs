...
export const ModelForm = (props : ModelFormProps) => {
 ...
 const [currentlyEditing, changeCurrentlyEditing] = useState<ModelObject | null> (null);
 const [newCount, changeNewCount] = useState<number>(0);
 const [createObject, changeCreateObject] = useState<ModelObject>(props.defaultObject != null ? { ...props.defaultObject } : {});
 ...
 useEffect(()=>{
   doApiRequest(
     `${props.baseUrl}?format=json${props.listParams ?? ''}`,
   )
     .then((resp) => resp.json())
     .then((resp) => {
       changeObjects(resp);
