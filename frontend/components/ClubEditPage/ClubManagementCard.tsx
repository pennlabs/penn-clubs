...
const ClubManagementCard = ({
  ...
  const reloadOwned = () => {
    doApiRequest(`/clubs/${club.code}/owned_affiliations/?format=json`)
      .then((resp) => resp.json())
      .then((affiliations) => {
  ...
  useEffect(() => {
    if (activeAffiliation != null) {
      doApiRequest(`/affiliations/${activeAffiliation.id}/clubs/?format=json`)
        .then((resp) => resp.json())
        .then(setClubs)
    } else {
      setClubs(null)
                    ...
                    <div className="mb-5">
                      <Formik
                        onSubmit={(
                          ...
                          doApiRequest(
                            `/affiliations/${activeAffiliation.id}/clubs/`,
                            {
                              method: 'POST',
                              body: data,
                            },
                          ...
                          <div style={{ marginLeft: 'auto' }}>
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                setLoading(true)
                                doApiRequest(
                                  `/affiliations/${activeAffiliation.id}/clubs/${club.code}/?format=json`,
                                  { method: 'DELETE' },
                                ).then(() => {
                                  toast.success(
                                    <>
